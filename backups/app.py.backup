...
if not (ROOT_DIR / "outputs").exists():
...
html, body, .gradio-container {
  ...
  background:
    radial-gradient(circle at 8% 0%, rgba(124, 58, 237, 0.26), transparent 30%),
...
def build_app():
    ...
    with gr.Blocks(title="Shadow Supervisor — Ultra Premium Demo") as demo:
        gr.HTML(hero_html())
        ...
        with gr.Tabs():
            with gr.Tab("🔥 Judge Demo"):
                ...
                demo.load(
                    fn=lambda: scenario_bundle(default_scenario),
                    inputs=None,
                    outputs=scenario_outputs,
                )
                ...
                table = gr.Dataframe(
                    label="Side-by-side supervisor result",
                    interactive=False,
                    wrap=True,
                )
            ...
            with gr.Tab("🎯 Live Single Policy"):
                ...
                run_single.click(
                    fn=run_live_demo,
                    inputs=[policy_choice, scenario_choice],
                    outputs=[single_summary, single_trace],
                )
            ...
            with gr.Tab("📊 Policy Comparison"):
                ...
                gr.Dataframe(
                    value=policy_comparison_dataframe(),
